...
# Proxy the sockets
rm -f /run/vpp/read-punt.socket /run/vpp/punt.socket
socat TCP-LISTEN:"${VPP_SSWAN_PORT}",reuseaddr,fork UNIX-SENDTO:/run/vpp/read-punt.socket &
socat UNIX-RECVFROM:/run/vpp/punt.socket,reuseaddr,fork TCP-CONNECT:"${VPP_SERVER_IP}":"${VPP_SERVER_PORT}" &
