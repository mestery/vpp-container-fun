...
docker run --privileged --mac-address "${VPP_SERVER_MAC}" --net "${VPP_DOCKER_NETWORK}" --ip "${VPP_SERVER_IP}" --cap-add IPC_LOCK --cap-add NET_ADMIN --env-file ./env.list -id --name vppvpnserver "${SERVERIMAGE}"
